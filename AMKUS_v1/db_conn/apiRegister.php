...
function patientCheckIn($referredBy, $pTemp, $pBP, $pWeight, $pHeight, $pId, $attendingDoc)
{
    ...
    $sql = "INSERT INTO `prescription`(`prescription_id`, `patient_id`,`name`, `age`, `gender`, `phone`, `address`, `attending_doctor`, `referred_by`, `height`, `weight`, `blood_pressure`, `temperature`, `status`,`visit_date`) VALUES ('{$newPID}','{$pId}','{$pName}','{$pAge}','{$pGender}','{$phone}','{$pAddress}','{$attendingDoc}','{$referredBy}','{$pHeight}','{$pWeight}','{$pBP}','{$pTemp}','checked_in',NOW())";
